...
void update_view(int i, int j, double direction, t_map *map)
{
	...
	view->view_angle_y = 2 * atan(map->height / (map->width * tan(view->view_angle_x /2)));
}
...
t_point *step_intersect(t_line line, double angle, int step, t_view *view)
{
	...
	if ((angle > M_PI_4 && angle < M_PI_2) || (angle > M_PI_2 && angle <
	M_PI_2 + M_PI_4))
	{
		yl = view->y0 + step - 0.5;
	...
	if ((angle > M_PI + M_PI_4 && angle < M_PI + M_PI_2) || (angle > M_PI +
	M_PI_2 && angle < M_PI + M_PI_2 + M_PI_4))
	{
		yl = view->y0 - step + 0.5;
		xl = (line.n * yl - line.m) / line.k;
		i = (int) trunc(xl);
